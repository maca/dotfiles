...
add_binds("normal", {
    ...
    buf("^,s$",                     function (w, c) w:enter_cmd(":open s ") end),
    buf("^,S$",                     function (w, c) w:enter_cmd(":tabopen s ") end),
    ...
    key({"Mod1"},    "Left",        function (w, m) w.tabs:reorder(w.view, w.tabs:current() - m.count) end, {count=1}),
    key({"Mod1"},    "Right",       function (w, m) w.tabs:reorder(w.view, (w.tabs:current() + m.count) % w.tabs:count()) end, {count=1}),
